...
$\hat{\pi}_{0\bullet}, \hat{\pi}_{\bullet0}, \hat{\pi}_0$ are estimated by applying Jin and Cai's method (@jin2007estimating) to$P_{\alpha_j},P_{\beta_j},P_{\max_j}$.
...
Note that there is one internal node (Node144) with p-value being NA. The reason is that the treatment variable at this node is all equals 
...
demo.rsltlst <- phyloMed(Trt, M, Y, tree = tree, f
                         dr.alpha = 0.1, graph = "circular")
# take a look at phyloseq-class object
